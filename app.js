const state = {
  ...
  todos: [],
  kitState: { version: 1, flight: [], days: {} }
};
...
function kitStorageKey() {
  return `travel-plan:kits:v1:${encodeURIComponent(state.data?.metadata?.tripId || "default-trip")}`;
}

function defaultKitState() {
  const flightItems = state.data?.flightJourneys?.[0]?.comfortItems || [];
  return {
    version: 1,
    flight: flightItems.map((text, index) => ({ id: `flight-kit-${index + 1}`, text, completed: false })),
    days: Object.fromEntries((state.data?.days || []).map((day) => [String(day.day), (day.prepItems || []).map((item, index) => ({
      id: item.id || `day-${day.day}-prep-${index + 1}`,
...
function loadKitState() {
  const fallback = defaultKitState();
  try {
    const stored = JSON.parse(localStorage.getItem(kitStorageKey()) || "null");
    if (!stored || stored.version !== 1) throw new Error("No kit state");
    state.kitState = {
      version: 1,
      flight: Array.isArray(stored.flight) ? stored.flight : fallback.flight,
      days: { ...fallback.days, ...(stored.days && typeof stored.days === "object" ? stored.days : {}) }
    };
  } catch {
    state.kitState = fallback;
  }
}

function saveKitState() {
  try { localStorage.setItem(kitStorageKey(), JSON.stringify(state.kitState)); }
  catch (error) { console.warn("Could not save kit state", error); }
}

function kitItems(scope, day) {
  if (scope === "flight") return state.kitState.flight;
  const key = String(day);
  if (!Array.isArray(state.kitState.days[key])) state.kitState.days[key] = [];
  return state.kitState.days[key];
}

function editableKitMarkup(scope, day, title) {
  const items = kitItems(scope, day);
  const completed = items.filter((item) => item.completed).length;
  const attributes = `data-kit-scope="${escapeHtml(scope)}"${day ? ` data-kit-day="${day}"` : ""}`;
  return `
    <details class="editable-kit" ${attributes}>
      <summary><span>${escapeHtml(title)}</span><b>${completed}/${items.length}</b></summary>
      <div class="editable-kit__body">
        <div class="editable-kit__list">
          ${items.map((item) => `
            <div class="editable-kit__item${item.completed ? " is-complete" : ""}" data-kit-id="${escapeHtml(item.id)}">
              <label>
                <input type="checkbox" ${item.completed ? "checked" : ""}>
                <span class="editable-kit__check">✓</span>
                <span>${escapeHtml(item.text)}</span>
              </label>
              <button type="button" data-kit-delete aria-label="删除${escapeHtml(item.text)}">删除</button>
            </div>`).join("") || `<p class="editable-kit__empty">暂无物品，可以在下方添加。</p>`}
        </div>
        <form class="editable-kit__form">
          <input type="text" maxlength="50" placeholder="新增一个物品" aria-label="新增物品">
...
function refreshKit(container) {
  const scope = container.dataset.kitScope;
  const day = container.dataset.kitDay;
  const title = scope === "flight" ? "飞机舒适包" : "出门准备";
  const wasOpen = container.open;
  const wrapper = document.createElement("div");
  wrapper.innerHTML = editableKitMarkup(scope, day, title);
  const next = wrapper.firstElementChild;
  next.open = wasOpen;
  container.replaceWith(next);
}

function setupEditableKits() {
  document.addEventListener("change", (event) => {
    const checkbox = event.target.closest(".editable-kit__item input[type='checkbox']");
    if (!checkbox) return;
    const container = checkbox.closest(".editable-kit");
    const item = kitItems(container.dataset.kitScope, container.dataset.kitDay).find((entry) => entry.id === checkbox.closest("[data-kit-id]").dataset.kitId);
    if (!item) return;
    item.completed = checkbox.checked;
    saveKitState();
    refreshKit(container);
  });
  document.addEventListener("click", (event) => {
    const button = event.target.closest("[data-kit-delete]");
    if (!button) return;
    const container = button.closest(".editable-kit");
    const id = button.closest("[data-kit-id]").dataset.kitId;
    const items = kitItems(container.dataset.kitScope, container.dataset.kitDay);
  ...
  document.addEventListener("submit", (event) => {
    const form = event.target.closest(".editable-kit__form");
    if (!form) return;
    event.preventDefault();
    const input = form.querySelector("input");
    const text = input.value.trim();
    if (!text) return;
    const container = form.closest(".editable-kit");
    kitItems(container.dataset.kitScope, container.dataset.kitDay).push({ id: `kit-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, text, completed: false });
    saveKitState();
    refreshKit(container);
  });
}
...
function flightCard(journey, index) {
  ...
  return `
    <article class="flight-card" data-journey="${escapeHtml(journey.id)}">
      ...
      ${editableKitMarkup("flight", null, "飞机舒适包")}
      ${journey.comfortNote ? `<p class="flight-comfort-note">${escapeHtml(journey.comfortNote)}</p>` : ""}
      <div class="flight-card__countdown-row">
  ...
  return `
    <article class="day-card${isToday ? " is-today" : ""}" data-day="${day.day}">
      ...
      <div class="day-detail" id="day-detail-${day.day}" ${expanded ? "" : "hidden"}>
        ${editableKitMarkup("day", day.day, "出门准备")}
...
async function loadSharedState() {
  ...
  if (shouldReseedPacking) {
    ...
    await Promise.all([
      ...
      ...state.todos.map((todo) => todoAdapter.applyChange("todos", todo, "upsert"))
    ]);
  }
}
